...
tags:
  ...
  - blitter
---
Last month development recap. Not really an over-productive month, but some interesting things have been achieved.

The configuration-file support is something that I reworked several times. I began with a JSON file, but I didn't like to use a third-party library just for parsing a simple configuration file. So, I moved to a Lua script... which made sense since Lua is the scripting language I'm using for the engine (courios that, Sol, Lua's ancestor, was development many years ago mostly as a configuration language), but it over-complicated the engine code and gave no real advantage over non-interpreted configuration files. So, I settled to the basic and plain key-value format for which a custom parsed can be coded in few lines. Over the last week I reworked the file format once again, toward something more similar to [`.ini` files](https://en.wikipedia.org/wiki/INI_file). This is a format that, despite being a very simple format, I always liked to use because it just makes things done. No fuss. Also it's interesting the fact that "regions" (called "contexts" in `#tofuengine`) are used to group related parameters. This goes along with the recent refactoring, were I refactored the code to use *nameless structs* as a way to (again) group related parameters and make the code clearer.
...
As a minor optimization, an [OpenGL's vertex array object](https://www.khronos.org/opengl/wiki/Vertex_Specification) is used to perform the (one and only) texture draw operation, i.e. the one that moves the visual data from an offscreen texture to the frame-buffer. While this change doesn't seem to boost performances significantly, it make nonetheless the code cleaner.

At last the most time-consuming activity for this month worth of coding.

I (finally) fixed the roto-scaling algorithm for good (or should I call it [roto-zoomer](https://mrdoob.com/lab/javascript/effects/rotozoomer/00/)?). It was bugged since the very beginning, as with a proper combination of actual argument, a pixel row and/or column would be missing due in the final result to rounding errors. I knew it. But, despite having pretty clear the origin of the issue, I wasn't just able to fix the *optimized* algorithm... so I had to opt for a different route.

For the sake of my mental health (and since this isn't going to be most significant part of the game engine), I went for a simpler/naive technique... but that handles rounding errors in a stabler manner! Yep, you guessed right: I'm (inverse) rotating by applying the rotation formula to each pixel individually.
...
If the anchor point is set to a a corner, for example, the radius it is the full diagonal length of the sprite rectangle.

Then, we just iterate across the disc's axis-aligned bounding-box (and we inverse roto-scale the pixels). As an optimization, we calculate the distance between each pixel and the disc's center, in order to discard the pixels that lay outside the disc and save a useless operation (since roto-scaled pixels are tested for inclusion within the source texture and they would end being discarded eventually).
...
These aren't the best roto-scaling and scaling algorithm by any stretch, but for the moment being, I'm happy with the result. Perhaps I'll go back and rewrite/optimize/fix this part of code once more, in the future... but that's more than enough for a small 2D game-engine. If I were to use prominently rotated and scaled sprites, I'll write a different rendering system that full exploits the GPU rather than being entirely software based.
